...
import { browserEngine } from "./tts-browser";

// ── Types ──────────────────────────────────────────────

export type TTSEngineType = "voicevox" | "sbv2" | "browser";
...
function stripSpacesBetweenCJK(text: string): string {
  let prev = "";
  while (prev !== text) {
    prev = text;
...
export async function speak(text: string, options?: SpeakOptions): Promise<void> {
  ...
  text = stripSpacesBetweenCJK(stripEmoji(text));
  if (!text) return;

  const sentences = splitIntoSentences(text);
  if (sentences.length <= 1) {
    const audioData = await engine.synthesize(text, voiceId);
    if (!engine.handlesPlayback) {
      await playAudio(audioData, playOpts);
    }
    return;
  }

  // If the engine handles its own playback, play them sequentially so utterance.onend can resolve
  if (engine.handlesPlayback) {
    const token = cancelledToken;
    for (const s of sentences) {
      if (cancelledToken !== token) return;
      await engine.synthesize(s, voiceId);
    }
    return;
  }

  // Kick off all synthesis requests in parallel for external engines
  const token = cancelledToken;
