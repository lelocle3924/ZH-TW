...
const TOCFL_GUIDELINES: Record<string, string> = {
  Novice: `TOCFL Novice (Beginner):
- Use only basic vocabulary (~500 words) and simple grammar
- Stick to present/past tense, basic sentence structures
- Use common Traditional Chinese characters, avoid rare idioms and complex words
- Short, simple sentences
- Topics: self-introduction, daily routines, basic shopping, weather`,

  "Band A": `TOCFL Band A (Elementary):
- Use elementary vocabulary (~1,000 words) and grammar
- Can use simple conjunctions and common measure words
- Use basic Traditional characters 
- Sentences can be slightly longer with simple conjunctions
- Topics: hobbies, travel plans, giving/receiving, making requests`,

  "Band B": `TOCFL Band B (Intermediate):
- Use intermediate vocabulary (~2,500-5,000 words)
- Can use more complex grammar, idioms (chengyu) sparingly
- Use intermediate Traditional characters
- Natural sentence structures with multiple clauses
- Topics: opinions, explanations, news, workplace situations`,

  "Band C": `TOCFL Band C (Advanced):
- Use native-level vocabulary and expressions
- All grammar patterns including literary and formal styles
- Full Traditional character usage and common Taiwanese slang/idioms
- Natural, nuanced expression, appropriate register
...
/**
 * Get system prompt for Taiwanese Mandarin conversation practice based on TOCFL level
 */
export function getConversationSystemPrompt(
  tocflLevel: string = "Novice",
  autoAdjust: boolean = false,
  responseLength: ResponseLength = "natural"
): string {
  const levelGuidelines = TOCFL_GUIDELINES[tocflLevel] || TOCFL_GUIDELINES.Novice;
  ...
  return `You are a native Taiwanese Mandarin speaker helping someone practice conversational Taiwanese Mandarin. You are currently role-playing in a scenario.
...
IMPORTANT RULES:
1. ALWAYS respond in Traditional Chinese (zh-TW) with Taiwanese Mandarin usage only. Never include Pinyin, Zhuyin (Bopomofo), or English translations unless the student explicitly asks for help in English.
2. Match your language complexity to the student's TOCFL level described above.
3. Only switch to English if the student writes in English to ask a question (e.g. "How do I say...?"). In that case, briefly answer in English, then return to Traditional Chinese.
...
7. Do NOT add parenthetical translations, pronunciations, or English glosses. The student has a translate button they can use.
8. Write Traditional Chinese text CONTINUOUSLY without spaces between words, exactly as natural Chinese is written. Only use full-width punctuation (。，！？) to separate clauses — NEVER insert spaces between Chinese words.
...
export function buildScenarioPrompt(
  ...
  const basePrompt = getConversationSystemPrompt(tocflLevel, autoAdjust, responseLength);
...
CURRENT SCENARIO:
Title: ${scenario.title} (${scenario.title_zh})
...
export async function translateToEnglish(chineseText: string): Promise<string> {
  const systemPrompt =
    "You are a Traditional Chinese to English translator. Translate the given Taiwanese Mandarin text to natural English. Only output the translation, nothing else.";
  const provider = getLLMProvider();
...
export async function generateFeedback(
  ...
  const systemPrompt = `You are a Taiwanese Mandarin language teacher who reviews practice conversation transcripts and returns structured JSON feedback. You NEVER continue the conversation — you only analyze it. Return ONLY valid JSON, no markdown fences, no extra text.`;
...
Return your analysis as JSON in exactly this format:
{
  ...
  "vocabulary": [
    { "word": "Traditional Chinese word", "reading": "Zhuyin or Pinyin reading", "meaning": "English meaning", "example": "example sentence using the word", "source_session": "${new Date().toISOString().split("T")[0]}" }
  ],
...
export function buildOngoingChatPrompt(
  ...
  return `You are ${chat.persona}. You are a Taiwanese friend having a casual ongoing conversation in Traditional Chinese. You and the student have been chatting over many sessions — this is a continuous relationship, not a one-off practice exercise.
...
IMPORTANT RULES:
1. ALWAYS respond in Traditional Chinese (zh-TW) only. Never include Pinyin, Zhuyin, or English translations unless the student explicitly asks for help in English.
2. Match your language complexity to the student's TOCFL level described above.
3. Only switch to English if the student writes in English to ask a question (e.g. "How do I say...?"). In that case, briefly answer in English, then return to Traditional Chinese.
...
export async function summarizeConversation(
  ...
  const systemPrompt = `You are a conversation summarizer. You will receive a previous summary (if any) and a block of conversation messages between a Chinese language student and their AI conversation partner. Produce a concise updated summary that captures:
