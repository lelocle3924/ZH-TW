...
function rowToProfile(row: ProfileRow): UserProfile {
  return {
    tocfl_level: (row.jlpt_level as any) === "N5" ? "Novice" : ((row.jlpt_level as any) || "Novice") as UserProfile["tocfl_level"],
...
export async function addCustomScenario(
  ...
  await d.execute(
    `INSERT INTO custom_scenarios (id, title, title_zh, description, setting, character_role, objectives, custom_prompt)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
    [
      id, scenario.title, scenario.title_zh, scenario.description,
...
export async function updateCustomScenario(
  ...
  await d.execute(
    `UPDATE custom_scenarios SET
      title = $1, title_zh = $2, description = $3, setting = $4,
      character_role = $5, objectives = $6, custom_prompt = $7
    WHERE id = $8`,
    [
      updated.title, updated.title_zh, updated.description, updated.setting,
...
export async function clearAllData(): Promise<void> {
  ...
  await d.execute("UPDATE user_profile SET jlpt_level='Novice', auto_adjust_level=0, estimated_level='beginner', response_length='natural', user_name=NULL, age=NULL, about_you=NULL, interests='[]', topics_covered='[]', recent_struggles='[]', total_sessions=0, voicevox_speaker_id=NULL, voicevox_speaker_name=NULL WHERE id=1");
}
  ...
  if (profileJson) {
    try {
      ...
      await d.execute(
        ...
        WHERE id=1`,
        [
          p.tocfl_level, p.auto_adjust_level ? 1 : 0, p.estimated_level,
  ...
  if (scenariosJson) {
    try {
      ...
      for (const sc of scenarios) {
        await d.execute(
          `INSERT OR IGNORE INTO custom_scenarios (id, title, title_zh, description, setting, character_role, objectives, custom_prompt)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
          [sc.id, sc.title, sc.title_zh, sc.description, sc.setting, sc.character_role, JSON.stringify(sc.objectives), sc.custom_prompt ?? null]
        );
      }
      migrated = true;
    } catch { /* skip */ }
