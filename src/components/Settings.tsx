...
import type { TOCFLLevel, ResponseLength } from "@/types";

const TOCFL_LEVELS: { value: TOCFLLevel; label: string; description: string }[] = [
  { value: "Novice", label: "Novice (Band Novice)", description: "Basic phrases, ~300 words, survival Chinese" },
  { value: "Band A", label: "Band A (Elementary)", description: "Basic conversations, ~500-1000 words" },
  { value: "Band B", label: "Band B (Intermediate)", description: "Everyday situations, ~2500-5000 words" },
  { value: "Band C", label: "Band C (Advanced)", description: "Academic/professional usage, native level, ~8000+ words" },
];
...
export function Settings() {
  ...
  const [tocflLevel, setTocflLevel] = useState<TOCFLLevel>("Novice");
  ...
  const selectedTocflInfo = useMemo(
    () => TOCFL_LEVELS.find((level) => level.value === tocflLevel),
    [tocflLevel]
  );
  ...
  useEffect(() => {
    ...
    getUserProfile().then((profile) => {
      setTocflLevel(profile.tocfl_level);
  ...
  const handleEngineChange = (engine: TTSEngineType) => {
    ...
    setMessage({ type: "success", text: `TTS engine set to ${engineNames[engine]}` });
    setTimeout(() => setMessage(null), 3000);
  };
  ...
  const handleTocflChange = async (level: TOCFLLevel) => {
    setTocflLevel(level);
    await updateUserProfile({ tocfl_level: level });
    setMessage({ type: "success", text: `TOCFL level set to ${level}` });
    setTimeout(() => setMessage(null), 3000);
  };
  ...
  return (
    <div className="h-full bg-background p-4 overflow-auto">
      <div className="max-w-lg mx-auto space-y-6 pb-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">TOCFL Level</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Set your current Taiwanese Mandarin proficiency level. The AI will adjust vocabulary and grammar complexity accordingly.
            </p>

            <div className="space-y-1.5">
              <label htmlFor="tocfl-level-select" className="text-sm font-medium">
                Current Level
              </label>
              <select
                id="tocfl-level-select"
                value={tocflLevel}
                onChange={(e) => handleTocflChange(e.target.value as TOCFLLevel)}
              ...
              {selectedTocflInfo && (
                <p className="text-xs text-muted-foreground">{selectedTocflInfo.description}</p>
              )}
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {ttsEngine === "voicevox" && <VoicevoxControl />}
            {ttsEngine === "sbv2" && <SBV2Control />}
            {ttsEngine === "browser" && (
              <div className="text-sm text-muted-foreground">
                Using the browser's native text-to-speech engine. Voices depend on your operating system and browser. Make sure to test the voices below to confirm they work.
              </div>
            )}
          </CardContent>
        </Card>
